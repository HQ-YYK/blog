...
import { SizesFun, EventBus, FunSerialization } from '@/hooks/Utils'
...
const Index = forwardRef(
  ({ raycaster, sounds }: HoverIconProps, ref: React.Ref<any>) => {
    const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 })
    const [clicked, setClicked] = useState(false)
    const [clickCTAVisible, setClickCTAVisible] = useState(false)
    const [closed, setClosed] = useState(false)
    const [isHoveringCursorElement, setIsHoveringCursorElement] =
      useState(false)
    ...
    const playIntro = async (modelDB) => {
      const landingPageFunString = sessionStorage.getItem('landingPageFun')
      const landingPageFun =
        landingPageFunString &&
        JSON.parse(landingPageFunString, FunSerialization().funReviver)
      console.log(landingPageFun)

      const getRoomModel = await DB().getDataByKey(
        modelDB,
    ...
    useEffect(() => {
      ...
      colorSwitchContainerElements = document.getElementById(
      ...
      return () => {
        ...
        removeCursorLeavesDoc()
        removeEventListeners()
        removeColorSwitchEventListeners()
      }
    }, [raycaster])
    ...
    const setupDefault = () => {
      if (
        currentIcon != 'default' &&
        !isHoveringCursorElement &&
        !raycaster.isHovering
      ) {
        currentIcon = 'default'
      ...
      if (currentIcon != 'pointer' && target) {
        ...
        if (
          ...
          setTimeout(() => {
            ...
            hoverIconElements.style.borderColor = target.color
              ? target.color
              : '#091434'
    ...
    const setupCircle = (
      target: { type: string; color?: string },
      element: any
    ) => {
      if (currentIcon !== 'circle') {
        ...
        if (isForce || !isActiveMenuItem) {
          ...
          hoverIconElements.style.background = isForce
            ? '#FF923E'
            : color
            ? color
            : ''
          ...
          element.addEventListener('mouseenter', () => {
            if (!sizes.touch) {
              hoverElement.type == 'pointer'
                ? setupPointer(hoverElement, element)
                : setupCircle(hoverElement, element)
    ...
    const updateBaseColor = (color: string, isVisible?: boolean) => {
      ...
      setTimeout(() => {
        ...
        const landingPageVisible = landingPageFun.visible

        if (
          isVisible ||
          (!hidden &&
            (cursorIsInsideDoc || landingPageVisible) &&
    ...
    const applyColorSwitchEventListeners = () => {
      ...
      colorSwitchContainerElements.addEventListener('mousemove', () =>
        handleColorSwitchEnter()
      )
      colorSwitchContainerElements.addEventListener('mousenter', () =>
        handleColorSwitchEnter()
      )
      aboutSectionElements.addEventListener('mousemove', () =>
        handleColorSwitchEnter()
      )
      aboutSectionElements.addEventListener('mouseenter', () =>
        handleColorSwitchEnter()
      )
      colorSwitchContainerElements.addEventListener('mouseleave', () =>
        handleColorSwitchLeave()
      )
      aboutSectionElements.addEventListener('mouseleave', () =>
        handleColorSwitchLeave()
      )
    }

    const removeColorSwitchEventListeners = () => {
      const handleColorSwitchEnter = () => {
        updateBaseColor('#34bfff')
      }
      ...
      colorSwitchContainerElements.removeEventListener('mousemove', () =>
        handleColorSwitchEnter()
      )
      colorSwitchContainerElements.removeEventListener('mousenter', () =>
        handleColorSwitchEnter()
      )
      aboutSectionElements.removeEventListener('mousemove', () =>
        handleColorSwitchEnter()
      )
      aboutSectionElements.removeEventListener('mouseenter', () =>
        handleColorSwitchEnter()
      )
      colorSwitchContainerElements.removeEventListener('mouseleave', () =>
        handleColorSwitchLeave()
      )
      aboutSectionElements.removeEventListener('mouseleave', () =>
        handleColorSwitchLeave()
      )
    }
